...
- ✅ **Update without losing the web** (final phase): `kumo update` swaps the
  ...
  restore is deferred — see 0.4.0). The daemon execs the new binary in place
  (`daemon --resume`),
...
- **Keymap data-driven**: the hard-coded leader dispatch (`leader_command` in
  `src/app.rs`) becomes a configurable table; remap any binding (splits, focus,
  sessions, zoom, sidebar, …). The same table feeds the dispatch, the
  leader-mode hint, and the `leader+?` showcase, so they never drift. Mouse
  gestures stay fixed: drag-resize, the context menu, and selection are
  positional hit-testing, not key sequences, so they are deliberately **not**
  remappable. While the dispatch becomes a table, the **missing stock
  bindings** land too: **keyboard resize** (`leader+H/J/K/L` — today only
  mouse-drag resize exists), swap/rotate panes, and show-pane-numbers
  (`leader+q`).
- **Custom leader keys** and per-mode keymaps (leader / popup — normal mode is
  pure passthrough to the PTY, so it has no command surface to remap).
- **Config expansion** (`src/config.rs`): `keymap`, `leader`, and
  `follow-workspace`; **clear validation errors** instead of silent ignores.
...
- **Follow workspace** — the daemon holds each pane's cwd via **OSC 7**
  ...
  snippet into your shell rc (zsh / bash / fish, with confirmation). The snippet
  is **idempotent and reversible**, and skips shells that already emit OSC 7
  (several distros / Oh My Zsh do).

**Deferred from 0.4.0**: full screen+scrollback restore after update/restart
(the re-encode-as-ANSI replay is lossy and only fires on update/restart —
processes and layout already survive today), OSC 133 semantic prompts (only
0.7.0's command traceback consumes them), status-bar layout (lands whole with
0.5.0's widgets), and the control CLI / scripting (`kumo send-keys`,
