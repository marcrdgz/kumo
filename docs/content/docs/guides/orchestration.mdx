...
| `traceback` | Last prompt block: `form` (below last rule) or last 120 rows if empty — structured command boundaries are planned for 0.8 |
...
`--ai` creates an ephemeral worktree and opens a session for it. The
...
or a GitHub/GitLab pull or merge request URL. Jira references are not supported
yet. `--agent KIND` starts the selected agent in the new pane; it does not send
the task text.

In 0.7, submit the task as a separate explicit step after startup:
...
Inspect or clean up the worktree with `kumo worktree current`, `list`,
...
Automatic task submission and the fully chained
create→start→ready→prompt flow are planned for 0.7.1.
...
Use the same primitives for a manual verify loop: run the suite into a fresh
split, wait for its output, and only then feed the failure to the agent:
...
Kumo ships its orchestration instructions inside the binary, so they always
match the installed CLI:

<AgentSkillInstaller />

The command does not require a running daemon. The selector installs the skill
