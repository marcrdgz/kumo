...
impl App {
    ...
    fn layout_node(&self, node: &crate::layout::Node) -> kumo_protocol::LayoutNode {
        ...
        match node {
            ...
            crate::layout::Node::Split { id, dir, ratio, a, b } => LN::Split {
                id: *id,
                dir: match dir {
...
/// Low-level cell helpers shared with the clients that draw chrome. The daemon
/// itself never calls them; they live here (not in `kumo_protocol`) because
/// they depend on `ratatui`.
pub(crate) fn put(f: &mut Frame, x: u16, y: u16, ch: &str, style: Style) {
    let a = f.area();
...
pub(crate) fn text(f: &mut Frame, x: u16, y: u16, s: &str, style: Style, max_width: u16) {
    for (i, ch) in s.chars().take(max_width as usize).enumerate() {
        put(f, x + i as u16, y, &ch.to_string(), style);
    }
}

pub(crate) fn fill(f: &mut Frame, area: Rect, color: RColor) {
