...
use kumo_protocol::{
    ...
    SessionLayout, SplitDir, WireBranch, WireCell,
    WireWorktree,
};

use crate::app::bindings::{self, Action, Binding, Chord};
use crate::app::ui::{fill, put, text};
use crate::app::{link_modifiers, Dir};
use crate::layout::{self, PaneGeom, TreeGeom};
...
impl View {
    ...
    fn send(&mut self, cmd: &Command) -> Result<()> {
        crate::protocol::write_framed(&mut self.out, cmd)?;
        Ok(())
    }
    ...
    fn recompute_geometry(&mut self) {
        ...
        self.splitters = geom
            ...
            .map(|s| {
                let dir = match s.dir {
                    crate::layout::SplitDir::V => SplitDir::Vertical,
                    crate::layout::SplitDir::H => SplitDir::Horizontal,
                };
    ...
    fn run_action(&mut self, action: Action) -> Result<()> {
        ...
        match action {
            ...
            Action::Resize(dir) => {
                let dir = match dir {
                    crate::layout::ResizeDir::Left => kumo_protocol::ResizeDir::Left,
                    crate::layout::ResizeDir::Down => kumo_protocol::ResizeDir::Down,
                    crate::layout::ResizeDir::Up => kumo_protocol::ResizeDir::Up,
                    crate::layout::ResizeDir::Right => kumo_protocol::ResizeDir::Right,
                };
    ...
    fn open_worktree_popup(&mut self, idx: usize) {
        ...
        if ws.as_deref().and_then(crate::app::worktrees::repo_root).is_none() {
            self.notice = Some((
            ...
            MouseEventKind::Up(MouseButton::Left) => {
                ...
                if let Some(pc) = self.pending_click.take() {
                    ...
                    if sel.start != sel.end {
                        ...
                        if !text.is_empty() {
                            crate::app::util::copy_to_clipboard(&text);
    ...
    fn render_settings_about(&self, f: &mut Frame, dd: Rect) {
        ...
        let rows: [(&str, String); 5] = [
            ("version", format!("{} ({})", env!("CARGO_PKG_VERSION"), crate::update::current_channel_label())),
            ("session", session),
...
fn wire_to_layout(node: &LayoutNode) -> crate::layout::Node {
    match node {
        LayoutNode::Pane(p) => crate::layout::Node::Pane { id: p.id },
        LayoutNode::Split { id, dir, ratio, a, b } => crate::layout::Node::Split {
            id: *id,
            dir: match dir {
                SplitDir::Vertical => crate::layout::SplitDir::V,
                SplitDir::Horizontal => crate::layout::SplitDir::H,
            },
            ratio: *ratio,
...
fn render_item_row(f: &mut Frame, x0: u16, y: u16, width: u16, item: &str, sel: bool, theme: &crate::theme::Theme) {
    let bg = if sel { theme.accent } else { theme.panel_sep };
...
fn draw_scrollbar(f: &mut Frame, x: u16, y_top: u16, region_h: u16, offset: usize, total: usize, theme: &crate::theme::Theme) {
